...
func homePage(w http.ResponseWriter, r *http.Request) {
	...
	printstring := "GET http://localhost:10000/all - prints all entries in the database.\nPOST http://localhost:10000/article -d '{id: int, title: string, description: string, content: string}' - create a new article \n"
	printstring = printstring + "PUT http://localhost:10000/article/{id} -d '{id: int, title: string, description: string, content: string}' - update an existing entry \n"
	printstring = printstring + "DELETE http://localhost:10000/article/{id} - detele an entry based on ID \n"
	printstring = printstring + "GET http://localhost:10000/article/{id} - returns an entry in json format based on ID \n"
